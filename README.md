...
#### To submit a bash script use the following command:
...
#### The next script requires the x-external-genomes.txt file. Here are the steps to create this file. Run these command in the directory where you have your anvio db files. 
...
#### We used the following bash script to conduct a normalization of gene counts, which outputs two text files: one with raw counts and the other with normalized counts. 
...
#### We repeated the process explained above for another set of genes, functional genes. The following line was used in a bash script to annotate our databases for fungenes.

    anvi-run-hmms -c assembly-dbs/s_${SAMPLE}.db -H /work/jennifer.bowen/DBs/all_fungene_anvio/ -T 30
...
#### To prepare a tree for anvio, we need to use the following code in phyloseq after creating the objects above, to make a tree for our phylogram

    fun_dist = vegdist(mat_dc, method = "bray")
    fun_clust = hclust(dist)
    plot(fun_clust, hang = -1, cex = 0.9)  #visual check of tree
    write.tree(as.phylo(clust), "FUNGENE-deep-core.tre") #saves tree file

#### In addition to using phyloseq, we used anvio to create a phlyogram and visualize trends within our data. We need to log to discovery in a special way here:

    ssh -L 8081:localhost:8081 username@login.discovery.neu.edu


#### To begin an interactive anvio session, first activate anvio

    conda activate anvio-6.2
    
#### Then run the following to begin your session

    anvi-interactive -d count matrix file -t tree file -p FUN_CAZY_rel.db --manual-mode --server-only --port-number 8081

#### Where FUN_CAZY_rel.db here is a empty, newly created file created to save the output of your session. To begin your session, use your web browser and type the following address into the search bar: localhost:8081

#### Once your session has been pulled up successfully, use the following resources to manipulate your phylogram and make a visual, or get more information about how to make a phylogam:

## https://merenlab.org/2016/10/27/high-resolution-figures/
...
###  Once your databases have been annotated, create a matrix of the hmm hits for each sample using the "x_run-anvio-hmm-matrix.shx" found in this repository.  We want to know how many hits there are for each gene in each sample.  To create this matrix, we just need to run the following bash script. 
